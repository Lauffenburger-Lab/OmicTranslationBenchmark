...
class VarDecoder(torch.nn.Module):
    def __init__(self, latent_dim, hidden_layers, out_dim, dropRate=0.1, activation=None, bias=True, loss='nb'):
        ...
        if loss not in ['nb', 'gauss','gamma']:
            raise Exception("The only allowed arguments for `loss` are `nb` or `gauss` or `gamma`")
        self.loss = loss
        self.bias = bias
        self.num_hidden_layers = len(hidden_layers)
        self.bn = torch.nn.ModuleList()
        ...
        self.dropout = torch.nn.Dropout(dropRate)
        if loss == 'gauss':
            self.act = torch.nn.Hardtanh(min_val=0.0, max_val=1.2)
        else:
    ...
    def forward(self, x):
        ...
        if self.loss == 'gauss':
            ...
            gene_means = self.out_mu(x)
            gene_vars = self.act(F.softplus(self.out_var(x)).add(1e-6))
        if self.loss == 'nb':
            # gene_means = F.softplus(output[:, :dim]).add(1e-3)
            # gene_vars = F.softplus(output[:, dim:]).add(1e-3)
        ...
        if self.loss == 'gamma':
            gene_means = F.softplus(self.relu(self.out_mu(x))).add(1e-4)
            gene_vars = F.softplus(self.relu(self.out_var(x))).add(1e-4)
