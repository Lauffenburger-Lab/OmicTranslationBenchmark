...
for i in range(model_params["no_folds"]):
    ...
    for e in range(0, NUM_EPOCHS):
        ...
        for j in range(maxLen):
            ...
            loss = loss_1 + loss_2 + model_params['similarity_reg'] * silimalityLoss + model_params['lambda_mi_loss'] * mi_loss + prior_loss - model_params[
                ...
                model_params['species_class_reg'])+ Vsp.Regularization(model_params["v_reg"])
    ...
    torch.save(species_classifier, '../results/models/species_classifier_%s.pt' % i)
    torch.save(Vsp, '../results/models/Vspecies_%s.pt' % i)
    torch.save(adverse_classifier, '../results/models/classifier_adverse_%s.pt' % i)
    torch.save(protection_classifier,'../results/models/protection_classifier_%s.pt' % i)
...
df_result.to_csv('../results/10foldvalidation_wholeModel_'+str(latent_dim)+'dim'+str(NUM_EPOCHS)+'ep_serology.csv')
# df_result = pd.read_csv('../results/10foldvalidation_wholeModel_'+str(latent_dim)+'dim'+str(NUM_EPOCHS)+'ep_serology.csv',index_col=0)

pear_matrix_primates = pd.DataFrame(pear_matrix_primates)
pear_matrix_primates.columns = primates_exprs.columns
# pear_matrix_primates = pd.read_csv('../results/10foldvalidation_decoder_'+str(latent_dim)+'dim'+str(NUM_EPOCHS)+'ep_perFeature_primates.csv',index_col=0)
...
plt.savefig('../results/perFeature_performance_decoder_'+str(latent_dim)+'dim'+str(NUM_EPOCHS)+'ep_primates.png', bbox_inches='tight',dpi=600)
pear_matrix_human = pd.DataFrame(pear_matrix_human)
pear_matrix_human.columns = human_exprs.columns
# pear_matrix_human = pd.read_csv('../results/10foldvalidation_decoder_'+str(latent_dim)+'dim'+str(NUM_EPOCHS)+'ep_perFeature_human.csv',index_col=0)
...
pear_matrix_primates_latent.to_csv('../results/10foldvalidation_encoders_'+str(latent_dim)+'dim'+str(NUM_EPOCHS)+'ep_perFeature_primates.csv')
...
#               ylabel='PC')
# #plt.xlim(0,1)
# plt.savefig('../results/perFeature_encoder_'+str(latent_dim)+'dim'+str(NUM_EPOCHS)+'ep_primates.png', bbox_inches='tight',dpi=600)
sns.clustermap(pear_matrix_primates_latent,cmap='RdBu_r', center=0,vmin=-0.75, vmax=0.75)
plt.savefig('../results/perFeature_PCA_avgCorrelation_'+str(latent_dim)+'dim'+str(NUM_EPOCHS)+'ep_primates.png', bbox_inches='tight',dpi=600)
...
pear_matrix_human_latent.to_csv('../results/10foldvalidation_encoders_'+str(latent_dim)+'dim'+str(NUM_EPOCHS)+'ep_perFeature_human.csv')
...
#               ylabel='PC')
# #plt.xlim(0,1)
# plt.savefig('../results/perFeature_encoder_'+str(latent_dim)+'dim'+str(NUM_EPOCHS)+'ep_human.png', bbox_inches='tight',dpi=600)
sns.clustermap(pear_matrix_human_latent,cmap='RdBu_r', center=0,vmin=-0.75, vmax=0.75)
plt.savefig('../results/perFeature_PCA_avgCorrelation_'+str(latent_dim)+'dim'+str(NUM_EPOCHS)+'ep_human.png', bbox_inches='tight',dpi=600)
