...
for (cell in unique(sigInfo$cell_iname)){
  ...
  dir.create(path = paste0('preprocessed_data/baselineCell/',cell))
  ...
  for (fold in folds){
    ...
    data.table::fwrite(train,paste0('preprocessed_data/baselineCell/',cell,'/train_',i,'.csv'))
    data.table::fwrite(val,paste0('preprocessed_data/baselineCell/',cell,'/val_',i,'.csv'))
