...
latent_dimensions = [256,128,64,32,16,8]
...
for latent_dim in latent_dimensions:
    ...
    Path('LatentDimAnalysis_lands/'+str(latent_dim)+'/models').mkdir(parents=True, exist_ok=True)
    valPear = []
    valPear_1 = []
    valPear_2 = []
    valF1 = []
    ...
    for i in range(model_params['no_folds']):
        ...
        torch.save(decoder_1.state_dict(),'LatentDimAnalysis_lands/'+str(latent_dim)+'/models/decoder_1_%s.pth'%i)
        torch.save(decoder_2.state_dict(),'LatentDimAnalysis_lands/'+str(latent_dim)+'/models/decoder_2_%s.pth'%i)
        torch.save(prior_d.state_dict(),'LatentDimAnalysis_lands/'+str(latent_dim)+'/models/priorDiscr_%s.pth'%i)
        torch.save(local_d.state_dict(),'LatentDimAnalysis_lands/'+str(latent_dim)+'/models/localDiscr_%s.pth'%i)
        torch.save(encoder_1.state_dict(),'LatentDimAnalysis_lands/'+str(latent_dim)+'/models/encoder_1_%s.pth'%i)
        torch.save(encoder_2.state_dict(),'LatentDimAnalysis_lands/'+str(latent_dim)+'/models/encoder_2_%s.pth'%i)
        torch.save(classifier.state_dict(),'LatentDimAnalysis_lands/'+str(latent_dim)+'/models/classifier_%s.pth'%i)
    ...
    df_result_all.to_csv('LatentDimAnalysis_lands/validation_results.csv')
