...
results_reconstruction2 <- data.frame()
...
p_scatter_1 <- ggscatter(results_reconstruction %>% filter(model=='CPA') %>% mutate(predicting = paste0('Predicting per cell type ',predicting)),
  ...
  xlab('cell counts in the data')+
  ylab(expression(R^2))+
  # ylim(c(0,1))+
  geom_hline(yintercept = 1,color='black',linetype='dashed')+
  geom_hline(yintercept = 0,color='black',linetype='dashed')+
  ggtitle('Reconstruction performance in human')+
...
p1 <- ggboxplot(results,
                x='model',y='averageCellr',color = 'model')  + xlab('')+ ylab(expression('average R'^2))+
