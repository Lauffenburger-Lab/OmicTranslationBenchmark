...
model_params = {'encoder_1_hiddens':[4096,2048,1024,768,512],
                'encoder_2_hiddens':[4096,2048,1024,768,512],
                'latent_dim': 512, # itan 512
                'decoder_1_hiddens':[512,768,1024,2048,4096],
                'decoder_2_hiddens':[512,768,1024,2048,4096],
                'final_dec_1':6144,
                'final_dec_2':6144,
                'dropout_decoder':0.1,
                'dropout_encoder':0.1,
                ...
                'V_dropout':0.25,
                'state_class_hidden':[512,256,128,64],#oxi to 64 kai to 1024 arxika
                ...
                'adv_class_drop':0.1,
                'no_adv_class':2,
                'cell_class_hidden':[512,256,128,64],
...
for fold in range(6,7):
    ...
    torch.save(encoder_human,'pre_models/encoder_humanv2_%s.pth'%fold)
    torch.save(encoder_mouse,'pre_models/encoder_mousev2_%s.pth'%fold)
    torch.save(prior_d,'pre_models/prior_dv2_%s.pth'%fold)
    torch.save(local_d,'pre_models/local_dv2_%s.pth'%fold)
    #torch.save(prior_d_2,'pre_models/prior2_d_%s.pth'%fold)
    #torch.save(local_d_2,'pre_models/local2_d_%s.pth'%fold)
    torch.save(classifier,'pre_models/classifier_diseasev2_%s.pth'%fold)
    torch.save(Vsp,'pre_models/Vspeciesv2_%s.pt'%fold)
    torch.save(Vcell,'pre_models/Vcellv2_%s.pt'%fold)
    torch.save(adverse_classifier,'pre_models/classifier_adversev2_%s.pt'%fold)
    torch.save(cell_adverse_classifier,'pre_models/classifier_cell_adversev2_%s.pt'%fold)
    torch.save(cell_classifier, 'pre_models/classifier_cellv2_%s.pth' % fold)
    torch.save(species_classifier,'pre_models/species_classifierv2_%s.pt'%fold)
    #torch.save(encoder_interm_1,'pre_models/encoder_interm_human_%s.pt'%fold)
    #torch.save(encoder_interm_2,'pre_models/encoder_interm_mouse_%s.pt'%fold)
    ...
    results.to_csv('results/pretrain_10foldvalidationResults_lungs_withV.csv')
print2log(results)
