...
class GammaDecoder(torch.nn.Module):
    def __init__(self, latent_dim, hidden_layers, out_dim,dropRate=0.1, activation=None, bias=True,max_k = 4, max_th = 5):
        ...
        if activation is not None:
            self.activation = activation
        self.dropout = torch.nn.Dropout(dropRate)
        self.act_kappa = torch.nn.Hardtanh(min_val=0.0, max_val=max_k)
        self.act_theta = torch.nn.Hardtanh(min_val=0.0, max_val=max_th)
    ...
    def forward(self, x):
        ...
        output_kappa = self.act_kappa(self.output_linear_kappa(x)) + 1e-03
        output_theta = self.act_theta(self.output_linear_theta(x)) + 1e-03
...
class GaussianDecoder(torch.nn.Module):
    def __init__(self, latent_dim, hidden_layers, out_dim,dropRate=0.1, activation=None, bias=True,max_mu = 15.,max_var = 1.):
        ...
        if activation is not None:
            self.activation = activation
        self.dropout = torch.nn.Dropout(dropRate)
        self.act_mu = torch.nn.Hardtanh(min_val= 0.0, max_val=max_mu)
        self.act_var = torch.nn.Hardtanh(min_val=0.0, max_val=max_var)
    ...
    def forward(self, x):
        ...
        output_mu = self.act_mu(output_mu)
        output_var = self.output_linear_var(x)
        output_var = torch.nn.functional.leaky_relu(output_var,negative_slope=0.001)
        output_var_pos =  torch.log(1 + torch.exp(output_var + 1e-03)) + 1e-06
        output_var_pos = self.act_var(output_var_pos)

        return output_mu,output_var_pos
...
class VarDecoder(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.loss == 'gauss':
            ...
            #gene_means = F.softplus(output[:, :dim]).add(1e-3)
            #gene_vars = F.softplus(output[:, dim:]).add(1e-3)
            gene_means = F.softplus(self.relu(self.out_mu(x))).add(1e-3)
            gene_vars = F.softplus(self.relu(self.out_var(x))).add(1e-3)
        if self.loss == 'nb':
            #gene_means = F.softplus(output[:, :dim]).add(1e-3)
            #gene_vars = F.softplus(output[:, dim:]).add(1e-3)
            gene_means = F.softplus(self.relu(self.out_mu(x))).add(1e-3)
